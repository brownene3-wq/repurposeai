...
function showToast(message, type = 'success') {
    ...
    // ===== GOOGLE DRIVE IMPORT =====
    // Task #59 — Google Drive shareable links. We pop a small inline
    // prompt for the link (no Picker SDK needed), POST it to the server
    // which downloads via Drive's public export URL, then walk the same
    // post-import lifecycle as YouTube/Dropbox: into Media library + V1.
    const googleDriveBtn = document.getElementById('googleDriveImportBtn');
    if (googleDriveBtn){
      googleDriveBtn.addEventListener('click', async function(){
        const url = prompt(
          ...
          'Note: Drive\u2019s public download path supports files up to ~100MB. ' +
          'For larger files, use the Upload button instead.'
        );
        if (!url || !url.trim()) return;
        if (!/drive\.google\.com|docs\.google\.com/i.test(url)){
          showToast('That doesn\u2019t look like a Google Drive link', 'error');
          return;
        }
        googleDriveBtn.disabled = true;
        const oldText = googleDriveBtn.textContent;
        googleDriveBtn.textContent = '\u23f3 Importing...';
        try {
          const resp = await fetch('/video-editor/google-drive-import', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
          ...
          videoPreviewArea.classList.add('has-video');
          document.getElementById('exportButton').disabled = false;
          // Same lifecycle as local uploads + the other URL importers (Task #58)
          try {
            if (typeof window.addUploadedMediaItem === 'function'){
              window.addUploadedMediaItem({
          ...
          showToast(err.message || 'Google Drive import failed', 'error');
        } finally {
          googleDriveBtn.disabled = false;
          googleDriveBtn.textContent = oldText;
        }
      });
    }
...
// ═════════════════════════════════════════════════════════════════════
// Task #59 — POST /video-editor/google-drive-import
// Imports a video from a Google Drive shareable link. Extracts the file
// ID from any of the common share URL formats:
//   https://drive.google.com/file/d/FILE_ID/view?usp=sharing
//   https://drive.google.com/open?id=FILE_ID
//   https://drive.google.com/uc?id=FILE_ID
// Then downloads via Drive's public export endpoint:
//   https://drive.google.com/uc?export=download&id=FILE_ID&confirm=t
//
// Works for files set to "Anyone with the link". Files behind auth/login
// will return an HTML "Sign in" page — we detect that (HTML content-type
// or HTML magic bytes) and return a clear error instead of saving the
// page bytes as a "video".
//
// Body: { url: string, name?: string }
// Returns: { success, filename, serveUrl, duration }
// ═════════════════════════════════════════════════════════════════════
router.post('/google-drive-import', requireAuth, async (req, res) => {
  try {
    ...
    // Extract the file ID from common share-URL formats
    var fileId = null;
    var m1 = /\/file\/d\/([A-Za-z0-9_-]{20,})/.exec(url);
    var m2 = /[?&]id=([A-Za-z0-9_-]{20,})/.exec(url);
    fileId = (m1 && m1[1]) || (m2 && m2[1]) || null;
    if (!fileId){
      return res.status(400).json({
        error: 'Could not parse a Drive file ID from that URL. Use a "Share" link like https://drive.google.com/file/d/<ID>/view'
      });
    }

    var requestedName = String((req.body || {}).name || '').trim();
    var safeBase = requestedName.replace(/[^A-Za-z0-9._ \-]+/g, '_').slice(0, 60) || 'gdrive_import';
    var ext = path.extname(safeBase).toLowerCase();
    if (!ext){ safeBase += '.mp4'; ext = '.mp4'; }
    var outputFilename = 'gdrive_' + Date.now() + '_' + req.user.id + '_' + safeBase;
    var outputPath = path.join(uploadDir, outputFilename);

    // Drive's public export URL (works for files shared with "Anyone with the link")
    var driveUrl = 'https://drive.google.com/uc?export=download&id=' + fileId + '&confirm=t';

    const https = require('https');
    function followingGet(u){
      return new Promise(function(resolve, reject){
        https.get(u, function(resp){
          if (resp.statusCode >= 300 && resp.statusCode < 400 && resp.headers.location){
            resp.resume();
            resolve(followingGet(resp.headers.location));
            return;
          }
          if (resp.statusCode !== 200){
            reject(new Error('Drive HTTP ' + resp.statusCode));
            return;
          }
          resolve(resp);
        }).on('error', reject);
    ...
    var resp;
    try {
      resp = await followingGet(driveUrl);
    } catch (e){
      return res.status(400).json({ error: 'Could not reach Google Drive: ' + e.message });
    }

    var ct = (resp.headers['content-type'] || '').toLowerCase();
    if (ct.indexOf('text/html') !== -1){
      // Drive returned a confirmation/auth page, not a video. Most common
      // causes: the file isn't shared with "Anyone with the link", or it's
      // larger than 100MB and Drive wants a virus-scan confirm.
      resp.resume();
      return res.status(400).json({
        error: 'Google Drive returned an HTML page instead of the file. Make sure the link is shared with "Anyone with the link" and the file is under 100MB, or use the Upload button to add it directly.'
      });
    }
    ...
    if (!fs.existsSync(outputPath) || fs.statSync(outputPath).size < 1024){
      try { fs.unlinkSync(outputPath); } catch(_){}
      return res.status(400).json({ error: 'Drive returned an empty / too-small response.' });
    }

    var metadata;
    try { metadata = await getVideoMetadata(outputPath); }
    catch(e){ metadata = { duration: 0 }; }
    ...
    res.json({
      ...
      serveUrl: '/video-editor/download/' + outputFilename,
      title: requestedName || 'Google Drive import'
    });
  } catch (err){
