...
const upload = multer({
  ...
  limits: { fileSize: 5 * 1024 * 1024 }, // 5 MB cap (logos are tiny in practice)
...
// the cookie). Deduped by content hash so duplicate templates share a row.
...
async function ensureLogoTable() {
  ...
  try {
    ...
    console.error('[brand-templates] ensureLogoTable failed:', e.message);
  }
}
...
router.put('/:id', requireAuth, upload.single('logo'), async (req, res) => {
  try {
    ...
    if (req.file && req.file.buffer) {
      ...
      // Client sent existing filename — keep it
...
// current user's cookie references that id. The cookie list is the 