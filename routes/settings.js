...
router.get('/', requireAuth, async (req, res) => {
  ...
  const hasInstagram = !!user.instagram_id;
  const hasLinkedIn = !!user.linkedin_id;
  const hasPinterest = !!user.pinterest_id;
  const hasYouTube = !!user.youtube_id;
  ...
  res.send(`
      ...
      <div class="main-content">
        ...
        <div class="settings-section active" id="section-profile">
          ...
          <div class="settings-card">
            ...
            <div class="info-row" style="align-items:center;margin-top:12px">
              <span class="label" style="display:flex;align-items:center;gap:8px">
                <svg width="20" height="20" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" fill="currentColor"/></svg>
                X / Twitter
              </span>
              ${hasTwitter ? `
                <span class="value" style="color:#1DA1F2;font-weight:600">Connected${user.twitter_username ? ' as @' + user.twitter_username : ''}</span>
                <button class="btn-save" onclick="disconnectPlatform('twitter')" style="background:#ef4444;margin-left:12px;padding:6px 16px;font-size:.8rem">Disconnect</button>
              ` : `
                <a href="/twitter/connect" class="btn-save" style="text-decoration:none;display:inline-block;padding:6px 16px;font-size:.8rem">Connect X</a>
              `}
            </div>
            ...
            <div class="info-row" style="align-items:center;margin-top:12px">
              ...
              ${hasInstagram ? `
                <span class="value" style="color:#E4405F;font-weight:600">Connected${user.instagram_username ? ' as @' + user.instagram_username : ''}</span>
                <button class="btn-save" onclick="disconnectPlatform('instagram')" style="background:#ef4444;margin-left:12px;padding:6px 16px;font-size:.8rem">Disconnect</button>
              ` : `
                <a href="/instagram/connect" class="btn-save" style="text-decoration:none;display:inline-block;padding:6px 16px;font-size:.8rem">Connect Instagram</a>
              `}
            </div>
            ...
            <div class="info-row" style="align-items:center;margin-top:12px">
              ...
              ${hasLinkedIn ? `
                <span class="value" style="color:#0A66C2;font-weight:600">Connected${user.linkedin_name ? ' as ' + user.linkedin_name : ''}</span>
                <button class="btn-save" onclick="disconnectPlatform('linkedin')" style="background:#ef4444;margin-left:12px;padding:6px 16px;font-size:.8rem">Disconnect</button>
              ` : `
                <a href="/linkedin/connect" class="btn-save" style="text-decoration:none;display:inline-block;padding:6px 16px;font-size:.8rem">Connect LinkedIn</a>
              `}
            </div>

            <div class="info-row" style="align-items:center;margin-top:12px">
              <span class="label" style="display:flex;align-items:center;gap:8px">
                <svg width="20" height="20" viewBox="0 0 24 24"><path d="M12.017 0C5.396 0 .029 5.367.029 11.987c0 5.079 3.158 9.417 7.618 11.162-.105-.949-.199-2.403.041-3.439.219-.937 1.406-5.957 1.406-5.957s-.359-.72-.359-1.781c0-1.668.967-2.914 2.171-2.914 1.023 0 1.518.769 1.518 1.69 0 1.029-.655 2.568-.994 3.995-.283 1.194.599 2.169 1.777 2.169 2.133 0 3.772-2.249 3.772-5.495 0-2.873-2.064-4.882-5.012-4.882-3.414 0-5.418 2.561-5.418 5.207 0 1.031.397 2.138.893 2.738a.36.36 0 01.083.345l-.333 1.36c-.053.22-.174.267-.402.161-1.499-.698-2.436-2.889-2.436-4.649 0-3.785 2.75-7.262 7.929-7.262 4.163 0 7.398 2.967 7.398 6.931 0 4.136-2.607 7.464-6.227 7.464-1.216 0-2.359-.631-2.75-1.378l-.748 2.853c-.271 1.043-1.002 2.35-1.492 3.146C9.57 23.812 10.763 24 12.017 24 18.635 24 24.003 18.633 24.003 12.013 24.003 5.393 18.635.028 12.017.028z" fill="currentColor"/></svg>
                Pinterest
              </span>
              ${hasPinterest ? `
                <span class="value" style="color:#E60023;font-weight:600">Connected${user.pinterest_username ? ' as @' + user.pinterest_username : ''}</span>
                <button class="btn-save" onclick="disconnectPlatform('pinterest')" style="background:#ef4444;margin-left:12px;padding:6px 16px;font-size:.8rem">Disconnect</button>
              ` : `
                <a href="/pinterest/connect" class="btn-save" style="text-decoration:none;display:inline-block;padding:6px 16px;font-size:.8rem">Connect Pinterest</a>
              `}
            </div>

            <div class="info-row" style="align-items:center;margin-top:12px">
              <span class="label" style="display:flex;align-items:center;gap:8px">
                <svg width="20" height="20" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" fill="currentColor"/></svg>
                YouTube
              </span>
              ${hasYouTube ? `
                <span class="value" style="color:#FF0000;font-weight:600">Connected${user.youtube_channel_name ? ' as ' + user.youtube_channel_name : ''}</span>
                <button class="btn-save" onclick="disconnectPlatform('youtube')" style="background:#ef4444;margin-left:12px;padding:6px 16px;font-size:.8rem">Disconnect</button>
              ` : `
                <a href="/youtube/connect" class="btn-save" style="text-decoration:none;display:inline-block;padding:6px 16px;font-size:.8rem">Connect YouTube</a>
              `}
            </div>
            ...
            <div class="info-row" style="align-items:center;margin-top:12px">
              ...
              ${hasFacebook ? `
                <span class="value" style="color:#1877F2;font-weight:600">Connected${user.facebook_page_name ? ' (' + user.facebook_page_name + ')' : ''}</span>
                <button class="btn-save" onclick="disconnectPlatform('facebook')" style="background:#ef4444;margin-left:12px;padding:6px 16px;font-size:.8rem">Disconnect</button>
              ` : `
                <a href="/facebook/connect" class="btn-save" style="text-decoration:none;display:inline-block;padding:6px 16px;font-size:.8rem">Connect Facebook</a>
              `}
            </div>
          </div>
    ...
    <script>
      ...
      async function disconnectPlatform(platform) {
        if (!confirm('Disconnect your ' + platform.charAt(0).toUpperCase() + platform.slice(1) + ' account?')) return;
        try {
          var res = await fetch('/' + platform + '/disconnect', { method: 'POST' });
          var data = await res.json();
          if (!res.ok) throw new Error(data.error || 'Failed');
