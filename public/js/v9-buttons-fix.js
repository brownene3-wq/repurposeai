...
// eb[5] Rotate
wire(eb[5],function(){
var s=sld('Angle',0,360,0,'Â°',function(v){window._vt.rotate=v;aT();});
...
window._vt.keyframes.forEach(function(k,i){html+='<div style="padding:4px;background:#2a2a4a;margin:4px 0;border-radius:4px;">KF#'+(i+1)+' @ '+k.time+'s (R:'+k.rotate+'Â° Z:'+k.zoom+')</div>';});
html+='</div>';
...
wire(ab[6],function(){
var ctx=initAudioContext();
var s1=sld('Bass',â12,12,0,'dB',function(v){window._eqL.gain.value=v;});
var s2=sld('Mid',â12,12,0,'dB',function(v){window._eqM.gain.value=v;});
var s3=sld('Treble',â12,12,0,'dB',function(v){window._eqH.gain.value=v;});
...
wire(ab[8],function(){
var ctx=initAudioContext();
var s1=sld('Threshold',â100,0,â24,'dB',function(v){window._comp.threshold.value=v;});
...
// fx[3] Stickers
wire(fx[3],function(){
var g=grd([{label:'ð Like',v:'ð'},{label:'ð¥ Fire',v:'ð¥'},{label:'â¤ï¸ Heart',v:'â¤ï¸'},{label:'â­ Star',v:'â­'},{label:'ð¯ 100',v:'ð¯'},{label:'ð Laugh',v:'ð'},{label:'â¡ï¸ Arrow',v:'â¡ï¸'},{label:'ð Bell',v:'ð'}],function(p){
